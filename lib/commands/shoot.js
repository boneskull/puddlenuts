import {coercePositiveInteger} from '../util';
import {shoot} from '../shoot.js';
import {log} from '../log';
import {oneLineTrim} from 'common-tags';
import {cameraOpts, watsonOpts} from './common-opts';

const MIN_LIMIT = 10;
const SUGGESTED_LIMIT = 50;

export const command = 'shoot <classifier> <classes..>';
  ...
  return yargs
    ...
    .options({
      negative: {
        ...
        type: 'boolean',
        description:
          'Include negative example class in training (will be final class)',
        group: 'Class',
        default: false
      },
      limit: {
    ...
    .check(argv => {
      if (argv.classes.length === 1 && !argv.negative) {
        throw new Error(
          `Specify two (2) positive classes (<class1> <class2>), OR one (1) positive class and the "negative" flag (<class1> --negative)`
        );
      }
      const {limit} = argv;
      if (limit < MIN_LIMIT) {
